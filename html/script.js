...
function showNotes() {
  ...
  notesObj.forEach(function (element, index) {
    html += `
        ...
        <div class="card">
            ...
            <p>${element.title}</p>
            <button onClick="deleteNote(this.id)" id="${index}" class="deleteBtn">Dispatch Product</button>
        </div>
        </div>`;
  });
  ...
  if (notesObj.length == 0) {
    insertNotes.innerHTML = `Nothing to show! Please click on "Schedule Order" button to add a new Order.`;
